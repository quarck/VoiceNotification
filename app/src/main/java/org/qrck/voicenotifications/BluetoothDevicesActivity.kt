...
class BlueboothDeviceListAdapter(val context: Context, var entries: Array<BlueboothDeviceListEntry>)
    ...
    inner class ViewHolder(itemView: View)
        ...
        init {
            view = itemView.findViewById<LinearLayout>(R.id.linearLayoutBluetoothDeviceListEntry)

            deviceAddr = view.findViewById<TextView>(R.id.textViewDeviceMac)
            deviceName = view.findViewById<CheckBox>(R.id.checkBoxBTDeviceSelection)
...
//    private lateinit var settings: Settings
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        recyclerView = findViewById<RecyclerView>(R.id.list_bt_devices)
        recyclerView.layoutManager = staggeredLayoutManager;
        recyclerView.adapter = adapter;

        noDevicesText = findViewById<TextView>(R.id.no_devices_text)
    ...
    override fun onResume() {
        ...
        scope.launch(Dispatchers.IO) {
            ...
            val triggers = bluetoothManager.storage.triggerDevices

            val triggersHash = triggers.toHashSet()
            val devices = bluetoothManager.pairedDevices?.map { BlueboothDeviceListEntry(triggersHash.contains(it.address), it)}?.toTypedArray()

            withContext(Dispatchers.Main) {
                if (devices != null) {
                    noDevicesText.visibility = if (devices.isNotEmpty()) View.GONE else View.VISIBLE
                    adapter.entries = devices
